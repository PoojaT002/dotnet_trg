create table customers(custid int,custname varchar(20),age int,custAddress varchar(20),custPhone varchar(10))
select*from customers

create table orders(custid int,orderid int,orderdate date,product varchar(10),price float,quantity int)
select*from orders

insert into customers values(1,'Ronit',25,'Hyd',903485648),(2,'Arohi',24,'Bang',846537289),(3,'Avyaansh',20,'Mumbai',763542789),(4,'Nakshika',26,'Delhi',983452673),(5,'Aryan',23,'Pune',907852316),(6,'Poo',24,'Gul',983027645)

insert into orders values(1,101,'2025-02-20','bag',4500,1),(2,102,'2025-03-25','Heels',2800,1),(3,103,'2025-02-16','Shampoo',3000,2),(4,104,'2025-04-05','Watch',5000,1),(5,105,'2025-06-22','bedsheet',550,1),(6,106,'2025-05-27','Crokery',3500,1),(7,107,'2025-07-04','jeans',1599,1),(8,108,'2025-09-09','dress',2999,1)

select*from orders
--1 Display the list of customers who resides in Bangalore
select*from customers where custAddress='Bang'

--2 Display the list of customers who does not resides in Bangalore or chennai
select*from customers where custAddress='Bang' and custAddress!='chennai'

--3 Display the list of customers who’s age is greater then 50 and does not resides in Bangalore
select*from customers where age>50 and custAddress!='Bang'
...
--8Display the list of customer who’s name
--a. Start with s
--b. Third character is c
--c. Ends with e
select*from customers where custname like 'S%' or custname like '__C%' or custname like '%E'
...
--100-1/1/2000
--500
select CONCAT(orderid,'-',orderdate) as [OrderID-Date],(price*quantity) as [total(price*quantity)] from orders
...
--Custid Price (sum of price)
--Count (count of qty)
--1 5000 3
--2 4000 9
--3 6700 6
select custid, sum(price) as [Price (sum of price)],count(quantity) as [Count (count of quantity)] from orders group by custid

--21 Display above details only for price > 4000
select custid, sum(price) as [Price (sum of price)],count(quantity) as [Count (count of quantity)] from orders group by custid having sum(price)>4000
